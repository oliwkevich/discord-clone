...
export default async function handler(
  ...
  try {
    ...
    if (!serverId || !channelId || !messageId) {
      return res
        .status(400)
        .json({ error: "serverId and channelId and message is required" });
    }
    ...
    if (!canModify) {
      return res
        .status(401)
        .json({ error: "You cant to edit/delete this message" });
    }
    ...
    console.log("[API]: Message edit error:", error);
    return res.status(500).json({ error: "Message edit error" });
  }
}
